...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Few-shot evaluation using standard support/query splits."
    )
    parser.add_argument(
        "--ways", type=int, default=5, help="Number of ways (classes) per task"
    )
    parser.add_argument(
        "--shots",
        type=int,
        default=5,
        help="Number of shots per class in the support set",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of adaptation steps (epochs) on the support set",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Number of tasks to sample and evaluate",
    )
    parser.add_argument(
...
def main():
    ...
    train_dataset = BUPTCBFaceDataset(
        mode="train",
        root="data/bupt_balanced_face/BUPT-CBFace-12",
        cache_images=False,
        force_new_split=False,
    )
    val_dataset = BUPTCBFaceDataset(
        mode="val",
        root="data/bupt_balanced_face/BUPT-CBFace-12",
        cache_images=False,
        force_new_split=False,
    )
    test_dataset = BUPTCBFaceDataset(
        mode="test",
        root="data/bupt_balanced_face/BUPT-CBFace-12",
        cache_images=False,
        force_new_split=False,
    ...
    tasks = l2l.data.Taskset(
        combined_meta, task_transforms=transforms, num_tasks=args.num_tasks
    )
    ...
    for i in range(args.num_tasks):
        ...
        support_data, support_labels, query_data, query_labels = split_support_query(
            data, labels, ways, shots
        )
        ...
        print(
            f"Task {i+1}/{args.num_tasks}: Query Accuracy = {query_acc.item()*100:.2f}%"
        )
